...
private:
    ...
    void* lastBlockDeallocatedPtr_[BLOCK_SIZE];
    ...
    bool isPtrInBlock(void* ptr, void* block) {
        return static_cast<int8_t*>(ptr) >= static_cast<int8_t*>(block) 
                && static_cast<int8_t*>(ptr) < static_cast<int8_t*>(block) 
...
public:
    ...
    void deallocate(void* ptr) {
        if (isPtrInBlock(ptr, blocks_.back())) {
            lastBlockDeallocatedPtr_[lastBlockDeallocatedPtrCount_] = ptr;
            ++lastBlockDeallocatedPtrCount_;
        }
        freeMemory_.push_back(ptr);
        if (blocks_.size() > 1 && freeMemory_.size() == (blocks_.size() - 1)
                * BLOCK_SIZE + lastBlockDeallocatedPtrCount_) {
            clear();
        }
    }
    ...
    void clear() {
        ...
        for (size_t i = 0; i < blocks_.size(); ++i) {
            if (i + 1 == blocks_.size() && lastBlockDeallocatedPtrCount_ < lastBlockPos_) {
                break;
            }
            ::operator delete(blocks_[i]);
        }
        void* lastBlock = blocks_.back();
        blocks_.clear();
        freeMemory_.clear();
        if (lastBlockDeallocatedPtrCount_ < lastBlockPos_) {
            blocks_.push_back(lastBlock);
            for (size_t i = 0; i < lastBlockDeallocatedPtrCount_; ++i) {
                freeMemory_.push_back(lastBlockDeallocatedPtr_[i]);
            }
        } else {
...
template<size_t ChunkSize>
const size_t FixedMemoryManager<ChunkSize>::BLOCK_SIZE = 16;
