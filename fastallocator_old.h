...
private:
    size_t chunkPosition = ChunkSize;
    std::vector<void*> memory;
    std::vector<size_t> chunkNum; 
...
public:
    ...
    FixedMemoryManager(const ThisManager&) {
        // chunkPosition = manager.chunkPosition;
        // memory = manager.memory;
    }

    void swap(ThisManager& /*manager*/) {
        //std::swap(chunkPosition, manager.chunkPosition);
        //std::swap(memory, manager.memory);
    }

    const ThisManager& operator=(const ThisManager&) {
        // ThisManager cp = manager;
        // swap(cp);
        return *this;
    }

    bool operator==(const ThisManager&) const {
        return false; //chunkPosition == manager.chunkPosition &&
        // memory == manager.memory && memoryMap == manager.memoryMap;
    }
